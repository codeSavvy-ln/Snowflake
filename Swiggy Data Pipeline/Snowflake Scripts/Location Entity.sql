...
SANDBOX.STAGE_SCH.LOCATION

--Creating a location tableSANDBOX.STAGE_SCH.LOCATION
...
on_error = abort_statement;    --It will abbort it in cases of error.
...
--Turn the schema to clean_schema
...
-- When any changes is being recorded in the source table, then it will update the source table with the target table
WHEN MATCHED AND (
...
-- If no change is recorded then insert operation will be performed.
